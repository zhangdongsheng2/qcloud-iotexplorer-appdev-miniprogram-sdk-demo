...
Page({
    ...
    this.unsubscribeAll = subscribeStore([
      {
        ...
        onInitAndChange: this.prepareData.bind(this),
      },
    ]);
  },
  ...
  async onCheckFirmwareUpgrade() {
    ...
    try {
      ...
      // 获取更新进度状态
      const { Status } = await describeDeviceFirmwareUpdateStatus({
        ProductId,
        DeviceName,
      });

      switch (Status) {
        case UpgradeStatus.USER_CONFIRM: // 等待用户确认升级
        case UpgradeStatus.FAIL: // 上次升级失败
          wx.showModal({
            title: '可升级固件',

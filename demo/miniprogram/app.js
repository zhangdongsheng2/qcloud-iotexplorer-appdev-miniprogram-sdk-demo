...
App({
  ...
  wsSubscribe() {
    ...
    subscribeStore([
      {
        getter: (state) => state.deviceList.concat(state.shareDeviceList),
        onInitAndChange: (deviceList, oldDeviceList) => {
  ...
  async getAccessToken() {
    ...
    if (APP_KEY === 'YOUR_APP_KEY_HERE') {
      throw { msg: '请在 miniprogram/app.js 文件中填写 APP_KEY', code: 'INVAILD_APP_KEY' };
    }
    ...
    // 云函数中调用应用端 API 获取 AccessToken
    // 也可以使用自己的后端服务器获取 AccessToken，请参见
    // https://cloud.tencent.com/document/product/1081/40781
    try {
      ...
      // 返回 AppGetTokenByWeiXin API 的响应
      return data.Data;
    } catch (err) {
      // 云函数部署指引
